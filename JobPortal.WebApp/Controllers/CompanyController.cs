...
namespace JobPortal.WebApp.Controllers
{
    ...
    public class CompanyController : Controller
    {
        ...
        public async Task<IActionResult> Detail(string slug, bool showMap = false)
        {
            ...
            var mapQueryParts = new List<string>();
            if (!string.IsNullOrWhiteSpace(employer.Location)) mapQueryParts.Add(employer.Location);
            if (!string.IsNullOrWhiteSpace(employer.Address)) mapQueryParts.Add(employer.Address);
            if (!string.IsNullOrWhiteSpace(employer.Province?.Name)) mapQueryParts.Add(employer.Province.Name);
            if (!string.IsNullOrWhiteSpace(employer.Country?.Name)) mapQueryParts.Add(employer.Country.Name);
            if (mapQueryParts.Count == 0) mapQueryParts.Add(employer.FullName ?? string.Empty);

            var mapQuery = string.Join(", ", mapQueryParts);
            var encodedQuery = Uri.EscapeDataString(mapQuery);
